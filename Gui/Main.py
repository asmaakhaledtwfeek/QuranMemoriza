...
class QuranApp(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.textbox = QtWidgets.QLineEdit(self.root)
        self.textbox.setFont(font_second)
        self.horizontalLayoutTop.addWidget(self.textbox)

        self.btnSearch = QtWidgets.QPushButton(self.root)
        self.btnSearch.setText('ابحث برقم الآية')
        self.btnSearch.setFont(font_second)
    
        self.horizontalLayoutTop.addWidget(self.btnSearch)  
        
        self.setCentralWidget(self.btnSearch)
        ...
        self.btnRecord = QtWidgets.QPushButton(self.root)
        self.btnRecord.setText('سمَع وصحح ')
        self.btnRecord.setFont(font_second)
        self.horizontalLayoutBottom.addWidget(self.btnRecord)
        ...
        self.btnSearch.clicked.connect(self.get_aya_by_number)
    ...
    def get_aya_by_number(self):
        gui_sura_list.clear()
        self.listWidget.clear()
        self.listWidget.scrollToTop()
        currentSoura=self.cb.currentText()
        info = Quran._get_sura_info_by_name1(currentSoura)
        '''print(info["verses"])'''
        input = self.textbox.text()
        verse=[]
        for s in info["verses"]:
            if str(s["id"]) == input:
                verse=[{"suraId":info["id"],"ayaId":input,"ayaText":s["text"]}]
                
                lbl = Aya(s["text"], s["id"], 1, QtCore.Qt.AlignCenter)
